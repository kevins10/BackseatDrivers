...
class DriverRideViewActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (passengers != null) {
            ...
            val N = passengers.size // total number of textviews to add

            for (i in 0 until N) {
                println("debug1: i: $i")
                val map: HashMap<String, String> = passengers[i] as HashMap<String, String>
                val passengerId: String? = map.get("id")
                val pickup: String? = map.get("pickup_location")
                println("debug1: id: $passengerId, pickup: $pickup")
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    val firstName = Queries().getFirstName(passengerId.toString())
                    val lastName = Queries().getLastName(passengerId.toString())
